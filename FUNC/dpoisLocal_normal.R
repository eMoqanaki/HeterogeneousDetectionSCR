...
#'
#' The \code{dpoisLocal_normal} distribution is a NIMBLE custom distribution which can be used to model 
#' the Poisson observations (x) of a single individual over a set of detectors defined by their 
...
#'
#' The \code{dpoisLocal_normal} distribution incorporates three features to increase computation efficiency:
#' \enumerate{
...
#' }
#' 
#' The \code{dpoisLocal_normal} distribution requires that the x- and y- coordinates should be scaled to the habitat (\code{\link{scaleCoordsToHabitatGrid}})
#' 
...
#' @param lambda Baseline detection probability used in the half-normal detection function.
...
#'         s[i, 2] ~ dunif(0, 100)
#'         z[i] ~ dbern(psi)
#'         y[i,1:maxDetNum] ~ dpoisLocal_normal(detNums,
#'                                                  detIndices,
#'                                                  lambda,
#'                                                  sigma,
#'                                                  s[i,1:2],
  ...
  run = function( x = double(1),
    ...
    for(r in 1:localTrapsNum[sID]){
      if(theseLocalTraps[r] == detIndices1[count]){ 
        ...
        logProb <- logProb + dpois(x[count], p, log = TRUE)
        count <- count + 1
      }else{
        d2 <- pow(trapCoords[theseLocalTraps[r],1] - s[1], 2) + pow(trapCoords[theseLocalTraps[r],2] - s[2], 2)
        p <- lambda * exp(alpha * d2)
...
rpoisLocal_normal <- nimbleFunction(
  run = function( n = double(0, default = 1),
    ...
    ## Specify return type
    returnType(double(1))
    if(n >= 0) stop("Random generation for the dpoisLocal_normal distribution is not currently supported")
    dummyOut <- detIndices
    return(dummyOut)
  })
...
registerDistributions(
  list(
    dpoisLocal_normal = list(
      BUGSdist = 'dpoisLocal_normal(detNums, detIndices, lambda, sigma, s, trapCoords, localTrapsIndices, localTrapsNum, resizeFactor, habitatGrid, indicator)',
